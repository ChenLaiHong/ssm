...
<script type="text/javascript">
	...
	function commentReview(state){
		...
		$.messager.confirm("系统提示","您确定要审核这<font color=red>"+selectedRows.length+"</font>条评论吗？",function(r){
			if(r){
				$.post("${APP_PATH}/admin/comment/review.do",{ids:ids,state:state},function(result){
					if(result.success){
	...
	function formatBlogTitle(val,row){
		if(val==null){
			...
			return "<a target='_blank' href='${APP_PATH}/blog/articles/"+val.id+".html'>"+val.title+"</a>";			
		}
	}
...
<table id="dg" title="评论审核管理" class="easyui-datagrid"
   ...
   url="${APP_PATH}/admin/comment/list.do?state=0" fit="true" toolbar="#tb">
   <thead>
   	<tr>
   		<th field="cb" checkbox="true" align="center"></th>
   		<th field="cid" width="20" align="center">编号</th>
   		<th field="blog" width="200" align="center" formatter="formatBlogTitle">博客标题</th>
   		<th field="author" width="100" align="center">用户名</th>
